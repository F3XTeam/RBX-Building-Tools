...
function Cheer.Bind(Source, ...)
	...
	local Destinations = (DestinationType == 'table') and Destination or { Destination }
	...
	local function CallDestination(...)
		...
		for _, Destination in ipairs(Destinations) do
			...
			if DestinationType == 'function' then
				Destination(Filter(...))

			-- If `Destination` is a link, update its structure
			elseif (DestinationType == 'userdata') and pcall(function () return #Destination end) then
				Destination('Update', Filter(...))

			-- If `Destination` is a TextBox, update it if not focused
			elseif (DestinationType == 'Instance') and Destination:IsA 'TextBox' and not Destination:IsFocused() then
				Destination.Text = Filter(...)

			-- If `Destination` is a TextLabel, update it
			elseif (DestinationType == 'Instance') and Destination:IsA 'TextLabel' then
				Destination.Text = Filter(...)

			-- If `Destination` is a value instance, update it
			elseif (DestinationType == 'Instance') and Destination.ClassName:match('Value$') then
	...
	function Binding.Trigger()
		...
		-- If `Source` is a link
		if SourceType == 'userdata' and pcall(function () return #Source end) then
			CallDestination(#Source)

		-- If `Source` is a text box
		elseif (SourceType == 'Instance') and Source:IsA 'TextBox' then
			CallDestination(Source.Text)

		-- If `Source` is a button
		elseif (SourceType == 'Instance') and Source:IsA 'GuiButton' then
			CallDestination()

		-- If `Source` is a value instance
		elseif (SourceType == 'Instance') and Source.ClassName:match('Value$') then
	...
	elseif (SourceType == 'RBXScriptSignal') or (SourceType == 'table' and Source.Wait and Source.Connect) then
